...
public abstract class PublicTransport implements Writeable {
    ...
    /**
     * Creates a new public transport object based on the given
     ...
     *
     * If {type} is bus, then {extra} should be the registration number of
     * the bus.
     * If {type} is train, then {extra} should be the carriage count of
     * the train.
     ...
     *         (according to the {@link PublicTransport#encode()}
     ...
     *            value.
     *         3. The route number given in the string does not match one of
     *         the given existingRoutes.
     ...
     *         5. A vehicle of type train whose {extra} part (i.e. carriage
     *            count) is not an integer value.
    ...
    public static PublicTransport decode(String transportString,
        ...
        // {"type", "id", "capacity", "routeNumber", "extra"}
        ...
        try {
            ...
            int routeNumber = Integer.parseInt(vehicleFeatures[3].trim());

            // the route the vehicle follows
            // will be imported into PublicTransport constructor as a parameter
            Route route;

            // all route numbers of the existing routes in the transport network
            List routeNumbers = new ArrayList();

            existingRoutes.forEach(r -> routeNumbers.add(r.getRouteNumber()));

            // the index of the route object specified by vehicle
            // in the existingRoutes
     ...
     *
     * A helper method for helping method
     * {@link PublicTransport#decode(String, List)} to create a new public
     * transport object.
     *
     ...
     * @param extra 1. If {type} is bus, then {extra} should be the
     ...
     *            match the type given in the transportString (e.g. a Bus
     *            referencing a TrainRoute).
     *         3. A vehicle of type train whose {extra} part (i.e. carriage
     *            count) is not an integer value.
     *         4. An error (i.e. EmptyRouteException or
     *            IncompatibleTypeException is encountered whilst adding the
     *            vehicle to its route
     */
    private static PublicTransport generateVehicle(String type, int id,
                                                   int capacity,
            ...
            switch (type.trim()) {
                ...
                case "train":
                    ...
                    PublicTransport train =
                            new Train(id, capacity, route, carriage);
                ...
                case "ferry":
                    ...
                    PublicTransport ferry =
                            new Ferry(id, capacity,route, extra);
     ...
     * and {route} is replaced by the route number of the route the vehicle
     * is on. For example:
     *
     * <p>bus,1,30,1
     *
