...
public class Network {
    ...
    public Network(String filename)
        ...
        try {
            input = Files.readAllLines(Paths.get("networks", filename));
            numOfStops = Integer.parseInt(input.get(0).trim());
            numOfRoutes = Integer.parseInt(input.get(numOfStops + 1).trim());
            numOfVehicles = Integer.parseInt(input.get(
                    numOfStops + numOfRoutes + 2).trim());
        ...
        // if any extra blank lines present in the file, any number of Stops,
        // Routes or Vehicles is negative, throws TransportFormatException
        if (input.contains("") || numOfStops < 0
                || numOfRoutes < 0 || numOfVehicles < 0) {
            throw new TransportFormatException();
        }

        try {
            int sumOfStopsRoutes = numOfStops + numOfRoutes;

            // the sum total of Stops' count, Routes' count and Vehicles' count
            int sumTotal = numOfStops + numOfRoutes + numOfVehicles;

            for (int i = 1; i <= numOfStops; i++) {
                stops.add(Stop.decode(input.get(i)));
            }

            for (int i = (numOfStops + 2); i < (sumOfStopsRoutes + 2); i++) {
                routes.add(Route.decode(input.get(i), stops));
            }

            for (int i = (sumOfStopsRoutes + 3); i < (sumTotal + 3); i++) {
                vehicles.add(PublicTransport.decode(input.get(i), routes));
            }
        } catch (Exception e) {
            throw new TransportFormatException();
        }
     ...
     * they were added to the network. This also applies to the routes and
     * the vehicles.
     *
     * If the given filename is null, the method should do nothing.
     *
     * @param filename The name of the file to save the network to.
     * @throws IOException If there are any IO errors whilst writing
     *                     to the file.
     */
    public void save(String filename) throws IOException {
        ...
        try {
            Files.write(Paths.get("networks", filename), output);
        } catch (Exception e) {
            throw new IOException();
        }
    }
