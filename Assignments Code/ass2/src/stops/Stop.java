...
public class Stop implements Writeable {
     ...
     *          {@link Stop#encode()} representation). This includes, but
     *          is not limited to:
     *          1. x- or y- coordinates that are not integer values.
     *          2. Any extra delimiters (:) being encountered whilst parsing.
     *          3. Any of the parts of the string being missing. This includes
     *             a missing stop name (as this is defined as invalid according
     *             to the Stop constructor).
     */
    public static Stop decode(String stopString)
        ...
        // array with the format: { "name", "x", "y" }
        String[] stopFeatures = stopString.split(":");

        if (stopFeatures.length != 3) {
            throw new TransportFormatException();
        }

        try {
            String name = stopFeatures[0];
            int x = Integer.parseInt(stopFeatures[1].trim());
            int y = Integer.parseInt(stopFeatures[2].trim());
    ...
    /**
     * Encodes this stop as a string in the same format as specified
     * in {@link Stop#toString()}.
     *
